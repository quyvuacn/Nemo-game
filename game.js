...
const GAME_CONFIG = {
  ...
  // Player settings
  INITIAL_HEARTS: 5,
  BASKET_MOVE_DISTANCE: 75,
  BASKET_START_POSITION: 0,
  BASKET_MAX_POSITION: 375,
  BASKET_WIDTH: 75,
  BASKET_HEIGHT: 60,
...
class Heart {
  constructor(column, isSpecial = false) {
    this.x = column * GAME_CONFIG.BASKET_WIDTH;
    this.y = 0;
  ...
  update(deltaTime) {
    ...
    if (
      this.y + this.height > GAME_CONFIG.CANVAS_HEIGHT - GAME_CONFIG.BASKET_HEIGHT &&
      this.y < GAME_CONFIG.CANVAS_HEIGHT &&
      this.x === basketPosition
    ) {
      this.caught = true;
      const points = this.isSpecial ? GAME_CONFIG.SPECIAL_POINTS : GAME_CONFIG.NORMAL_POINTS;
      score += points;
  ...
  draw() {
    ...
    // Vẽ trái tim với đường cong mềm mại hơn
    ctx.beginPath();
    
    // Di chuyển điểm bắt đầu xuống thấp hơn một chút
    ctx.moveTo(centerX, centerY + this.height / 4);

    // Đường cong bên trái
    ctx.bezierCurveTo(
      centerX - this.width / 2,  // Điểm điều khiển 1 x
      centerY,                   // Điểm điều khiển 1 y
      centerX - this.width / 2,  // Điểm điều khiển 2 x
      centerY - this.height / 2, // Điểm điều khiển 2 y
      centerX,                   // Điểm đích x
      centerY - this.height / 3  // Điểm đích y
    );

    // Đường cong bên phải
    ctx.bezierCurveTo(
      centerX + this.width / 2,  // Điểm điều khiển 1 x
      centerY - this.height / 2, // Điểm điều khiển 1 y
      centerX + this.width / 2,  // Điểm điều khiển 2 x
      centerY,                   // Điểm điều khiển 2 y
      centerX,                   // Điểm đích x
      centerY + this.height / 4  // Điểm đích y
    );
    ...
    if (image && image.complete) {
      // Vẽ ảnh với một chút padding để tránh cạnh
      const padding = 2;
      ctx.drawImage(
        image, 
        this.x + padding, 
...
function drawBasket() {
  ...
  ctx.lineTo(basketPosition + GAME_CONFIG.BASKET_WIDTH - 5, GAME_CONFIG.CANVAS_HEIGHT);
